...
export function KpiBar({ todayNetProfit, todayRevenue, todayExpenses }: KpiBarProps) {
  return (
    <div className="grid grid-cols-3 gap-2 md:gap-4 mb-6">
      <FadeIn delay={0.1}>
        <Card className={`rounded-md md:rounded-xl border shadow-sm transition-all duration-300 hover:shadow-md h-full ${todayNetProfit >= 0 ? 'border-emerald-200 bg-emerald-50/50' : 'border-rose-200 bg-rose-50/50'}`}>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 p-2 sm:p-4 pb-0 sm:pb-2">
            <CardTitle className={`text-[9px] sm:text-sm lg:text-base font-bold uppercase tracking-wider whitespace-nowrap overflow-hidden text-ellipsis ${todayNetProfit >= 0 ? 'text-emerald-900' : 'text-rose-900'}`}>Today's Net Profit</CardTitle>
          ...
          <CardContent className="p-2 sm:p-4 pt-1 sm:pt-2">
            <div className={`text-xs sm:text-2xl lg:text-3xl font-bold tracking-tight truncate ${todayNetProfit >= 0 ? 'text-emerald-950' : 'text-rose-950'}`}>
      ...
      <FadeIn delay={0.2}>
        <Card className="rounded-md md:rounded-xl border border-emerald-200 bg-emerald-50/30 shadow-sm transition-all duration-300 hover:shadow-md h-full">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 p-2 sm:p-4 pb-0 sm:pb-2">
            <CardTitle className="text-[9px] sm:text-sm lg:text-base font-bold text-emerald-900 uppercase tracking-wider whitespace-nowrap overflow-hidden text-ellipsis">Today's Revenue</CardTitle>
      ...
      <FadeIn delay={0.3}>
        <Card className="rounded-md md:rounded-xl border border-zinc-200 bg-zinc-50/50 shadow-sm transition-all duration-300 hover:shadow-md h-full">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 p-2 sm:p-4 pb-0 sm:pb-2">
            <CardTitle className="text-[9px] sm:text-sm lg:text-base font-bold text-zinc-900 uppercase tracking-wider whitespace-nowrap overflow-hidden text-ellipsis">Today's Expenses</CardTitle>
